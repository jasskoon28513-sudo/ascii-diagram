...
def execute_ascii_diagram(query: str):
    """
    Skill: ASCII Diagram Generator - Generates ASCII art diagrams using the Gemini API.
    """
    if not model:
        raise Exception("AI model failed to initialize due to missing or invalid API key.")

    prompt = f"""
Render an ASCII diagram explaining the relationship between the classes, concepts, or elements described below.
Use a clear 60-character width limit.
Draw horizontal lines of underscores (____) at top and bottom.

FOCUS:
- Show how components interact.
- Highlight what’s new or changed.
- Prefer flowchart-style visuals if more intuitive.

If no document or content is provided, ask for one.
...
def execute():
    ...
    try:
        result = execute_ascii_diagram(query)
